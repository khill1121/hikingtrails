...
const hikingform = document.querySelector('#hikingform');
...
const hiking_form = document.querySelector('#hikes');
// show the DIV if it is hidden
hiking_form.style.display = 'block';


// event handler
hikingform.onsubmit = (e) => {
    // gets the names and current values from the form
    const formData = new FormData(hikingform);
    // FETCH
    fetch('enter.php', {
        method: "POST",
        body: formData,
    ...
    .then( (data) => {
        hiking_form.style.display = 'none';
